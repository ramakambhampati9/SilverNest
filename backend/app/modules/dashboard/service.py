...
class DashboardService:
    ...
    def get_summary(access_token: str, user_id: str) -> DashboardOut:
        ...
        history = DashboardRepository.list_snapshots(access_token, user_id)
        return DashboardOut(
            ...
            alerts=alerts,
            net_worth=float(history[-1]["net_worth"]) if history else float(profile_dict.get("existing_savings", 0) + profile_dict.get("existing_investments", 0)),
            networth_history=history,
        )

    @staticmethod
    def snapshot_all() -> int:
        return DashboardRepository.create_daily_snapshots()
